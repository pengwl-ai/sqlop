...
本文件详细介绍了SQL解析引擎项目的目录结构和各个文件的主要功能，帮助开发者快速了解项目组织和代码分布。最新测试结果显示，该引擎已成功处理867条SQL语句，解析成功率100%，性能达39万EPS。
...
├── resource_limited_test.rs # 资源受限环境测试工具
```
...
**resource_limited_test.rs** 用于在资源受限环境下测试引擎性能：
- 模拟低内存环境下的解析行为
- 测试CPU受限条件下的性能表现
- 评估引擎在资源紧张情况下的稳定性
...
该组件是解决SQL关键字错误识别问题的关键，性能测试显示可达到每秒处理39万条SQL语句的能力，远超6000 EPS的目标要求。最新测试已成功解析867条SQL语句，解析成功率达100%。
...
### src/bin/excel_sql_tester.rs

功能强大的Excel测试用例运行器，支持从Excel文件读取SQL测试用例并完整显示结果，是项目的主要测试工具。

### src/bin/resource_limited_test.rs

用于在资源受限环境下测试引擎性能和稳定性的专用工具。
